...
$(document).ready(function() {
    ...
    // duration value for video hard coded
    var MAGIC_DURATION = 33;
	  
	  
    //set up variables
    var speedSlow = false;
//    var video_dom = document.querySelector('#myPlayer');
    var video_dom = $("video#myPlayer");
    ...
    //var duration;
    var duration = $video_dom.attr('duration');
    ...
    $("form#submitReply").submit(function() {
        ...
        var parent_comment = $(this).children('.text_reply').attr('parent_id');
        var file = $(this).children('[name=reply-file-name]').val();
           
            
        $.ajax({
            type: "POST",
            url: "scripts/submit_reply.php",
            data: "reply="+ reply  +"&pID="+ parent_comment + "&file=" + file,
            dataType: "json",
    ...
    //toggles the optional text display for the source video
    ...
    traversalCanvas.addEventListener('mousedown', function(e) {
        ...
        if (relX < plusCircle.x + plusCircle.width 
            && relX > plusCircle.x - plusCircle.width 
            && !creatingTimedComment
            && relY < plusImage.height 
            && relY > 0 ) { //plusCircle.y - plusImage.height
    ...
    function calculateColorRegionPosition() {
        for(var i = signlinkArray.length - 1; i > -1; i--){
            if (video_dom.currentTime >= signlinkArray[i].startTime 
                && video_dom.currentTime <= signlinkArray[i].endTime) {
                //if the playhead is in a colored region
                return i;
            }
            else if (video_dom.currentTime >= signlinkArray[i - 1].endTime 
                && video_dom.currentTime <= signlinkArray[i].startTime) {
