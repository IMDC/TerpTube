...
function movePlayHead(){
    ...
    var currentTime = $('video#myPlayer').get(0).currentTime;
    var duration = $('video#myPlayer').get(0).duration;
...
function calculateColorRegionPosition() {
    for(var i = signlinkArray.length - 1; i > -1; i--){
        if($('video#myPlayer').get(0).currentTime >= signlinkArray[i].startTime && $('video#myPlayer').get(0).currentTime <= signlinkArray[i].endTime){
            //if the playhead is in a colored region
            return i;
        }
        else if($('video#myPlayer').get(0).currentTime >= signlinkArray[i - 1].endTime && $('video#myPlayer').get(0).currentTime <= signlinkArray[i].startTime){
            //if the playhead is not in a region does not cover if its the first 1
            return i;
        }
        else if($('video#myPlayer').get(0).currentTime <= signlinkArray[0].startTime){
            return 0;
        }
    }
}
...
*/
               ...
               function drawAreaOnBar(object) {
                   ...
                   var ctx=$('#linkCanvas')[0].getContext('2d');
                   var duration = $('video#myPlayer').get(0).duration;
               ...
               function createUploader($this){
                   var uploader = new qq.FileUploaderBasic({
                   	   multiple: false,
                   	   element:$('#uploadedfileButton')[0],
                       button: $('#input-upload-div')[0],
...
$(document).ready(function() {
        ...
        //set up variables
        var speedSlow = false;
   //     var $('video#myPlayer').get(0) = $('video#myPlayer').get(0)
        ...
        var linkCanvas=$('#linkCanvas')[0];
        var ctx=linkCanvas.getContext('2d');
        ctx.globalAlpha = 0.4;
        var traversalCanvas=$('#traversalCanvas')[0];
...
//        var duration;
      ...
      duration = $("video#myPlayer").get(0).duration;
        ...
        $(".clock-icon").click(function(){
            $('video#myPlayer').get(0).currentTime = $(this).attr("alt");
            movePlayHead();
        });
        ...
        $("#video-speed").click(function() {
            if(!speedSlow)
            {
                $('video#myPlayer').get(0).playbackRate = 0.5;
                speedSlow = true;
                $("#video-speed").attr("src", 'images/slowdown-pressed.png');
            }
            else{
                $('video#myPlayer').get(0).playbackRate = 1.0;
                speedSlow = false;
        ...
        $("#play-button").click(function() {
            if (playing){
                ...
                $('video#myPlayer').get(0).pause();
                playing = false;
            }
            else {
                $('video#myPlayer').get(0).play();
                playing = true;
        ...
        $("#closed-caption-button").click(function() {
            // alert($('video#myPlayer').get(0).tracks[0].mode);
            if($('video#myPlayer').get(0).tracks[0].mode == 2){
                $("#closed-caption-button").attr("src", 'images/closedCaptioning.jpg');
                $('video#myPlayer').get(0).tracks[0].mode = CAPTION_HIDDEN;
            }
            else
            {
                $("#closed-caption-button").attr("src", 'images/closedCaptioningDown.jpg');
                $('video#myPlayer').get(0).tracks[0].mode = CAPTION_SHOW;
        ...
        $("#video-link-forward-button").click(function() {
            ...
            {
                $('video#myPlayer').get(0).currentTime = signlinkArray[0].startTime;
            }
            else{
                if(playHeadInColoredRegion()){
                    $('video#myPlayer').get(0).currentTime = signlinkArray[regionPointer + 1].startTime;
                }
                else{
                    $('video#myPlayer').get(0).currentTime = signlinkArray[regionPointer].startTime;
                }
        ...
        $("#video-link-back-button").click(function() {
            ...
            if(!(regionPointer == 0) ) // if its not at the first link still
            {
                ...
                $('video#myPlayer').get(0).currentTime = signlinkArray[signlinkArray.length - 1].startTime;
            }
            animate();
            movePlayHead();
        });
        ...
        $('video#myPlayer').get(0).addEventListener('play', function() {
            
//            $('video#myPlayer').get(0).width = canvas_draw.width = $('video#myPlayer').get(0).offsetWidth;
//            $('video#myPlayer').get(0).height = canvas_draw.height = $('video#myPlayer').get(0).offsetHeight;
//            var ctx_draw = canvas_draw.getContext('2d');
            
            $('video#myPlayer').get(0).width = $('video#myPlayer').get(0).offsetWidth;
            $('video#myPlayer').get(0).height = $('video#myPlayer').get(0).offsetHeight;
        ...
        $('video#myPlayer').get(0).addEventListener('ended', function() {
            $("#play-button").attr("src", 'images/play_button.png');
            playing = false;

        }, false);

        $('video#myPlayer').get(0).addEventListener('pause', function() {
            interval = null;
            playing = false;
        ...
        traversalCanvas.addEventListener('mousedown', function(e) {
            ...
            if (relX < plusCircle.x + plusCircle.width && relX > plusCircle.x - plusCircle.width && !creatingTimedComment
                ...
                $(".comment-details").show();
                startTimeInput.val(roundNumber($('video#myPlayer').get(0).currentTime,2));
                endTimeInput.val(roundNumber($('video#myPlayer').get(0).currentTime + 2,2));
                ...
                if (relX < selectorRectLeft.x  && relX > selectorRectLeft.x - selectorLeftImg.width
                    ...
                    traversalCanvas.addEventListener('mousemove', function(e) {
                        ...
                        if(selectorLeftDrag && (e.clientX - offset.left < selectorRectRight.x - 3 )){
                            ...
                            $('video#myPlayer').get(0).currentTime = convertCanvasPointToTime(selectorRectLeft.x, duration);
                            startTimeInput.val(roundNumber($('video#myPlayer').get(0).currentTime,2));
                        }
                    }, true);
                }
                ...
                if (relX > selectorRectRight.x  && relX < selectorRectRight.x + selectorRightImg.width
                    ...
                    traversalCanvas.addEventListener('mousemove', function(e) {
                        ...
                        if(selectorRightDrag && (e.clientX - offset.left > selectorRectLeft.x + 1 )){
                            ...
                            $('video#myPlayer').get(0).currentTime = convertCanvasPointToTime(selectorRectRight.x, duration);
                            endTimeInput.val(roundNumber($('video#myPlayer').get(0).currentTime,2));
                ...
                $('video#myPlayer').get(0).currentTime = percentage * $('video#myPlayer').get(0).duration;
                movePlayHead();
                animate();
